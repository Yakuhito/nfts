...
use crate::utils::{bytes32_from_db, encode_launcher_bech32m, optional_bytes32_from_db};
...
pub async fn run(pool: &SqlitePool, args: ListArgs) -> Result<(), CliError> {
    ...
    if !args.nfts_only {
        ...
        for did in dids {
            let launcher_string = encode_launcher_bech32m(&did.launcher_id, "did:chia:")?;
    ...
    if !args.dids_only {
        ...
        for nft in nfts {
            let launcher_string = encode_launcher_bech32m(&nft.launcher_id, "nft")?;
            println!("- {launcher_string}");
        }
    }
...
async fn fetch_dids(pool: &SqlitePool) -> Result<Vec<Did>, CliError> {
    ...
    for row in rows {
        ...
        out.push(Did {
            ...
            parent_coin_id: optional_bytes32_from_db(
                "did parent_coin_id",
                row.get::<Option<&[u8]>, _>("parent_coin_id"),
            )?,
            coin_id: optional_bytes32_from_db(
                "did coin_id",
                row.get::<Option<&[u8]>, _>("coin_id"),
            )?,
            spent_height,
        });
    }
...
async fn fetch_nfts(pool: &SqlitePool) -> Result<Vec<Nft>, CliError> {
    ...
    for row in rows {
        ...
        out.push(Nft {
            ...
            inner_puzzle_hash: optional_bytes32_from_db(
